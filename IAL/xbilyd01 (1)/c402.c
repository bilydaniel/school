...
**/
	...
	{
		tmp = STopPopP(&ptr_stack);
		Leftmost_Preorder(tmp->RPtr, &ptr_stack);
	}
...
**/
	...
	while(!SEmptyP(&ptr_stack))
	{
		tmp = STopPopP(&ptr_stack);
		BTWorkOut(tmp);
		Leftmost_Inorder(tmp->RPtr, &ptr_stack);
	}
...
**/
	...
	{
		tmp = STopPopP(&ptr_stack);
		first_visit = STopPopB(&bool_stack);

		if(first_visit)
		{
			SPushP(&ptr_stack, tmp);
			SPushB(&bool_stack, FALSE);
			Leftmost_Postorder(tmp->RPtr, &ptr_stack, &bool_stack);
		}
		else
		{
			BTWorkOut(tmp);
...
**/
	...
	do
	{
		...
		{
			if(tmp->RPtr != NULL)
				SPushP(&ptr_stack, tmp->RPtr);

			to_be_removed = tmp;
			tmp = tmp->LPtr;
			free(to_be_removed);
		}
	}while((!SEmptyP(&ptr_stack))|| tmp != NULL);
