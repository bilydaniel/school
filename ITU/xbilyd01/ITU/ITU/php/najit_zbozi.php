...
if(isset($_POST['fraze'])){
	...
	$zbozi = $spravceZbozi->vratZboziPodleFraze($id, "nazev like ?", array("%" . $fraze . "%"));
}else{
	$nazev = $_POST['nazev'];	
	...
	$zbozi = $spravceZbozi->vratZboziPodleFraze($id, "nazev like ? and dodavatel like ? and mj like ?", array("%" . $nazev . "%", "%" . $dodavatel . "%", "%" . $mj . "%"));
}
