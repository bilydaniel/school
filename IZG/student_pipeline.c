...
void const *gpu_computeVertexAttributeDataPointer(
   ...
   assert(head != NULL);
  //buffer+offset+(stride*vertexID)
  return  (void const*)((size_t)head->buffer+ head->offset + head->stride * (size_t)gl_VertexID );
...
for(size_t i =0; i<MAX_ATTRIBUTES;i++)
{
  ...
  {
     output->attributes[i]=gpu_computeVertexAttributeDataPointer(head,gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  {
    gpu_runVertexPuller(&puller_output,puller,baseVertexShaderInvocation + i);
    shader_input.gl_VertexID=gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i);
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float denominator=((vertices[2].data[0] - vertices[0].data[0])*(vertices[2].data[1] - vertices[1].data[1])+
    (vertices[1].data[0] - vertices[2].data[0]) * (vertices[2].data[1] - vertices[0].data[1]));

  coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) / denominator;
  coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) / denominator;
  coords->data[2] = 1.0f - coords->data[1]-coords->data[0];
